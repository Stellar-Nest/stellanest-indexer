...
impl Dispatcher {
    ...
    async fn handle_index_update(&self, city: &str, value: f64, change: f64, sources: i32) {
        ...
        .bind(value).bind(change).bind(sources).bind(city)
        ...
        .bind(city).bind(value).bind(sources)
        .execute(&self.db).await {
            Ok(_) => {},
            Err(e) => { error!(error = %e, "failed to insert into index_history"); }
        }
        ...
        let payload = serde_json::to_string(&json!({
            "city": city, "value": value, "change_24h": change,
            "source_count": sources, "timestamp": chrono::Utc::now().to_rfc3339()
        })).unwrap_or_default();
        if let Err(e) = redis::cmd("PUBLISH").arg("stellanest:index_updates").arg(&payload)
            .query_async::<_, ()>(&mut *redis).await {
            warn!("failed to publish index update to Redis: {}", e);
        }
    }

    async fn handle_position_opened(&self, id: &str, wallet: &str, city: &str, direction: &str, size: f64) {
        match sqlx::query(
            "INSERT INTO positions (user_wallet, city_code, direction, size, status, soroban_position_id) VALUES ($1, $2, $3, $4, 'open', $5)"
        )
        .bind(wallet).bind(city).bind(direction).bind(size).bind(id)
        .execute(&self.db).await {
            Ok(_) => {},
            Err(e) => { error!(error = %e, "failed to insert into positions"); }
        }
    ...
    async fn handle_position_closed(&self, id: &str, pnl: f64) {
        ...
        .execute(&self.db).await {
            Ok(_) => {},
            Err(e) => { error!(error = %e, "failed to update positions"); }
        }
    ...
    async fn handle_liquidation(&self, id: &str, wallet: &str, city: &str, seized: f64, penalty: f64, tx: &str) {
        ...
        match sqlx::query(
            "INSERT INTO liquidations (position_id, user_wallet, city_code, collateral_seized, penalty, tx_hash) VALUES ($1, $2, $3, $4, $5, $6)"
        )
        .bind(id).bind(wallet).bind(city).bind(seized).bind(penalty).bind(tx)
        .execute(&self.db).await {
            Ok(_) => {},
            Err(e) => { error!(error = %e, "failed to insert into liquidations"); }
        }
        ...
        let mut redis = self.redis.lock().await;
        let payload = serde_json::to_string(&json!({
            "position_id": id, "user_wallet": wallet, "city_code": city,
            "collateral_seized": seized, "penalty": penalty, "tx_hash": tx,
            "timestamp": chrono::Utc::now().to_rfc3339()
        })).unwrap_or_default();
        if let Err(e) = redis::cmd("PUBLISH").arg("stellanest:liquidations").arg(&payload)
            .query_async::<_, ()>(&mut *redis).await {
            warn!("failed to publish liquidation to Redis: {}", e);
        }
    }
    ...
    async fn handle_trade(&self, city: &str, price: f64, size: f64, side: &str, tx: &str) {
        ...
        .bind(city).bind(price).bind(size).bind(side).bind(tx)
        .execute(&self.db).await {
            Ok(_) => {},
            Err(e) => { error!(error = %e, "failed to insert into trades"); }
        }

        info!(city, price, size, side, "trade executed");

        let mut redis = self.redis.lock().await;
        let payload = serde_json::to_string(&json!({
            "city_code": city, "price": price, "size": size,
            "side": side, "tx_hash": tx, "timestamp": chrono::Utc::now().to_rfc3339()
        })).unwrap_or_default();
        if let Err(e) = redis::cmd("PUBLISH").arg("stellanest:trades").arg(&payload)
            .query_async::<_, ()>(&mut *redis).await {
            warn!("failed to publish trade to Redis: {}", e);
        }
    }
}
